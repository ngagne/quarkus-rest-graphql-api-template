...
tools: ['read', 'edit', 'execute', 'search', 'web', 'io.qwen.upstash/context7/*', 'mcp__firecrawl__*', 'mcp__exa__*']
color: cyan
---
...
```bash
node ".qwen/get-shit-done/bin/gsd-tools.cjs" websearch "your query" --limit 10
```
...
```bash
node ".qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): research phase domain" --files "$PHASE_DIR/$PADDED_PHASE-RESEARCH.md"
```
