...
Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = .qwen/get-shit-done
                  → writes to .qwen/get-shit-done/hooks/
...
```bash
INIT=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" state load)
...
```bash
node ".qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```bash
node ".qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
