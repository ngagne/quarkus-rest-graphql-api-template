...
```bash
INIT=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" init execute-phase "${PHASE}")
...
node ".qwen/get-shit-done/bin/gsd-tools.cjs" state advance-plan

# Recalculate progress bar from disk state
node ".qwen/get-shit-done/bin/gsd-tools.cjs" state update-progress

# Record execution metrics
node ".qwen/get-shit-done/bin/gsd-tools.cjs" state record-metric \
  --phase "${PHASE}" --plan "${PLAN}" --duration "${DURATION}" \
  --tasks "${TASK_COUNT}" --files "${FILE_COUNT}"

# Add decisions (extract from SUMMARY.md key-decisions)
for decision in "${DECISIONS[@]}"; do
  node ".qwen/get-shit-done/bin/gsd-tools.cjs" state add-decision \
...
node ".qwen/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE_NUMBER}"

# Mark completed requirements from PLAN.md frontmatter
# Extract the `requirements` array from the plan's frontmatter, then mark each complete
node ".qwen/get-shit-done/bin/gsd-tools.cjs" requirements mark-complete ${REQ_IDS}
```
...
```bash
node ".qwen/get-shit-done/bin/gsd-tools.cjs" state add-blocker "Blocker description"
```
</state_updates>

<final_commit>
```bash
node ".qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs({phase}-{plan}): complete [plan-name] plan" --files .planning/phases/XX-name/{phase}-{plan}-SUMMARY.md .planning/STATE.md .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
