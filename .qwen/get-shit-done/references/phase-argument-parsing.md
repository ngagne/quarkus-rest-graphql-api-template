...
```bash
PHASE_INFO=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
