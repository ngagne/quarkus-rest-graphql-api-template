...
```bash
node ".qwen/get-shit-done/bin/gsd-tools.cjs" phases list --type summaries --raw
...
node ".qwen/get-shit-done/bin/gsd-tools.cjs" state add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>
...
```bash
node ".qwen/get-shit-done/bin/gsd-tools.cjs" roadmap update-plan-progress "${PHASE}"
```
...
```bash
node ".qwen/get-shit-done/bin/gsd-tools.cjs" commit "" --files .planning/codebase/*.md --amend
```
</step>
