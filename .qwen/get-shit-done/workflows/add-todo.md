...
```bash
INIT=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" init todos)
...
```bash
slug=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" generate-slug "$title" --raw)
```
...
```bash
node ".qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs: capture todo - [title]" --files .planning/todos/pending/[filename] .planning/STATE.md
```
