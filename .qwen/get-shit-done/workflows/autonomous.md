...
```bash
INIT=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" init milestone-op)
...
```bash
ROADMAP=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" roadmap analyze)
```
...
```bash
DETAIL=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
node ".qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```bash
UI_REVIEW_CFG=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```bash
node ".qwen/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
