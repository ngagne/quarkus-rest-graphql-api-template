...
```bash
INIT=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" init phase-op "${after_phase}")
...
```bash
RESULT=$(node ".qwen/get-shit-done/bin/gsd-tools.cjs" phase insert "${after_phase}" "${description}")
```
