...
Key references:
- @.qwen/get-shit-done/references/ui-brand.md (display patterns)
- @.qwen/get-shit-done/agents/gsd-user-profiler.md (profiler agent definition)
- @.qwen/get-shit-done/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH=".qwen/get-shit-done/USER-PROFILE.md"
...
```bash
cp ".qwen/get-shit-done/USER-PROFILE.md" ".qwen/get-shit-done/USER-PROFILE.backup.md"
```
...
✓ Stores profile at .qwen/get-shit-done/USER-PROFILE.md
...
Run session scan:
```bash
SCAN_RESULT=$(node .qwen/get-shit-done/bin/gsd-tools.cjs scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(node .qwen/get-shit-done/bin/gsd-tools.cjs profile-sample --json 2>/dev/null)
```
...
- The user-profiling reference doc at `.qwen/get-shit-done/references/user-profiling.md`
...
Reference: @.qwen/get-shit-done/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(node .qwen/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANALYSIS_RESULT=$(node .qwen/get-shit-done/bin/gsd-tools.cjs profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
node .qwen/get-shit-done/bin/gsd-tools.cjs write-profile --input "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Profile written to .qwen/get-shit-done/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  ...
  - "Global copilot-instructions.md" -- "Add profile to .qwen/copilot-instructions.md for all projects"

**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at .qwen/get-shit-done/USER-PROFILE.md" and jump to step 10.
...
```bash
node .qwen/get-shit-done/bin/gsd-tools.cjs generate-dev-preferences --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```

Display: "✓ Generated /gsd-dev-preferences at .qwen/commands/gsd/dev-preferences.md"

**For copilot-instructions.md profile section (if selected):**

```bash
node .qwen/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --json 2>/dev/null
```
...
```bash
node .qwen/get-shit-done/bin/gsd-tools.cjs generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json 2>/dev/null
```

Display: "✓ Added profile section to .qwen/copilot-instructions.md"
...
```bash
BACKUP_PATH=".qwen/get-shit-done/USER-PROFILE.backup.md"
```
...
Your profile:    .qwen/get-shit-done/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /gsd-dev-preferences   .qwen/commands/gsd/dev-preferences.md
  ✓ copilot-instructions.md section       ./copilot-instructions.md
  ✓ Global copilot-instructions.md        .qwen/copilot-instructions.md
```
